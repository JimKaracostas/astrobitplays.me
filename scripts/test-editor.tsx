...
test("Draft backups are account-scoped and malformed storage cannot crash the editor", () => {
  ...
  assert.equal(
    decodeBackup(JSON.stringify({ version: 1, form: { body: "only body" } })),
    null,
  );
  const backup = {
    version: 1,
    form: { ...emptyPost, title: "Unfinished article", body: "Unsaved text" },
  ...
  assert.equal(
    decodeBackup(
      JSON.stringify({ ...backup, form: { ...backup.form, score: "bad" } }),
    ),
    null,
  );
});
...
test("Formatting and image insertion preserve surrounding text and selection", () => {
  assert.deepEqual(insertText("Hello world!", 6, 11, "**", "**"), {
    text: "Hello **world**!",
    start: 8,
    end: 13,
  });
...
test("Only recognized video hosts are embedded", () => {
  ...
  assert.equal(
    youtubeId("https://www.youtube-nocookie.com/embed/abcdefghijk"),
    "abcdefghijk",
  );
  assert.equal(
    youtubeId("https://youtube.com/live/abcdefghijk?t=12"),
    "abcdefghijk",
  );
  assert.equal(
    youtubeId("https://evil.example/youtube.com/watch?v=abcdefghijk"),
    null,
  );
  ...
  const mixed = renderToStaticMarkup(
    <MarkdownContent
      content={"https://youtu.be/abcdefghijk https://example.com"}
    />,
  );
...
test("Cover validation agrees with the database HTTPS constraint", () => {
  ...
  assert.ok(
    validatePost({ ...input, cover_url: "http://example.com/image.png" }),
  );
  assert.ok(validatePost({ ...input, cover_url: "/logo.png" }));
  assert.equal(
    validatePost({ ...input, cover_url: "https://example.com/image.png" }),
    null,
  );
});
