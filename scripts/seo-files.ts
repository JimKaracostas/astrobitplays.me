...
export function sitemapXml(posts: SitemapPost[], now = new Date()) {
  ...
  function lastModified(items: SitemapPost[]) {
    const dates = items
      .map((post) => Date.parse(post.updated_at))
      .filter((date) => Number.isFinite(date) && date <= now.getTime());
    return dates.length
  ...
  const entries = [
    ...
    ...categories
      .filter((category) =>
        published.some((post) => post.category === category),
      )
      .map((category) => ({
    ...
    ...published.map((post) => ({
      url: articleUrl(post.slug, post.category),
      modified: lastModified([post]),
    })),
  ];
  if (entries.length > 50000)
    throw new Error(
      "Sitemap exceeds 50,000 URLs; split it into a sitemap index before deployment.",
    );
