-- Run once in the Supabase SQL editor. No demo content or automatic admin promotion.
begin;
...
create index posts_public_feed on public.posts(status, published_at desc);
...
create function private.stamp_post() returns trigger language plpgsql set search_path = '' as $$
begin
  new.updated_at = now();
...
create function public.record_post_view(post_id uuid, visitor_id uuid) returns void
...
create function public.post_stats() returns table(post_id uuid, total_views bigint, recent_views bigint)
