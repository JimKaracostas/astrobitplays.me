...
function Studio({ userId }: { userId: string }) { return <Suspense fallback={<p role="status">Loading dashboard…</p>}><LazyStudio userId={userId} /></Suspense> }
...
export function App() {
  ...
  const [posts, setPosts] = useState<Post[]>([]), [storedSaved, setSaved] = useState<string[]>([])
  ...
  useEffect(() => {
    ...
    Promise.all([supabase.rpc('is_owner'), supabase.from('bookmarks').select('post_id').eq('user_id', userId)]).then(([role, bookmarks]) => {
      ...
      setSaved(bookmarks.error ? [] : (bookmarks.data || []).map(item => item.post_id))
      if (role.error || bookmarks.error) setAccountError('Your account is signed in, but account features are not ready yet.')
    }).catch(() => { if (active) { setOwner(false); setSaved([]); setRoleUserId(userId); setAccountError('Account features are temporarily unavailable.') } })
    return () => { active = false }
  }, [userId])
  useEffect(() => {
    let active = true
  ...
  async function shareArticle() {
    ...
    } catch { setAccountError('The link couldn’t be copied. You can copy it from your browser’s address bar.') }
  }
  ...
  return <>
    ...
    <header className="site-header"><div className="header-inner">
      ...
      <div className="account-links">{!authReady ? <span className="session-loading">Checking account…</span> : user ? <><a href="/?page=saved">Saved</a>{owner && <a href="/?page=studio">Dashboard</a>}<button className="icon-button" onClick={signOut} aria-label="Sign out"><LogOut size={18} /></button></> : <button className="text-button" onClick={() => setSignIn(true)}>Sign in</button>}</div>
    </div></header>
...
function SignIn({ onClose }: { onClose: () => void }) {
  ...
  if (redirect.searchParams.get('page') === 'signin') redirect.searchParams.set('page', 'saved')
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (useOtp) {
        const { error: authError } = await database().auth.signInWithOtp({ email: email.trim(), options: { shouldCreateUser: mode === 'signup', emailRedirectTo: redirect.toString() } })
        if (authError) throw authError
        setMessage('Check your inbox for a sign-in link. You can close this window.')
      } else {
        if (mode === 'signup') {
          const { data, error: authError } = await database().auth.signUp({ email: email.trim(), password, options: { emailRedirectTo: redirect.toString() } })
          if (authError) throw authError
          if (data.session) { onClose() } else { setMessage('Check your email to confirm your account before signing in.') }
        } else {
