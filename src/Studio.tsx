...
import { backupKey, decodeBackup, insertText, markdownImage, postInput } from './lib/editor'
...
function errorMessage(error: unknown) {
  ...
  if (typeof error === 'object' && error && 'code' in error && error.code === 'PGRST116') return 'This post changed in another tab. Download your draft, then close and reopen the post to load the latest version.'
...
export function Studio({ userId }: { userId: string }) {
  ...
  const [restoreId] = useState(() => {
    try { return sessionStorage.getItem(activeKey) || sessionStorage.getItem('astrobit_active_edit') } catch { return null }
  })
  ...
  function setEditingTarget(target: Post | 'new' | null) {
    setEditing(target)
    try {
      if (target) sessionStorage.setItem(activeKey, target === 'new' ? 'new' : target.id)
  ...
  async function load() {
    ...
    try {
      ...
      setPosts(all); setPostsLoaded(true)
      if (pendingRestore.current) {
        const id = pendingRestore.current
        pendingRestore.current = null
        setEditing(id === 'new' ? 'new' : all.find(post => post.id === id) || null)
      }
  ...
  useEffect(() => { void load() }, [])
  if (editing) return <Editor key={typeof editing === 'string' ? 'new' : editing.id} userId={userId} post={editing === 'new' ? undefined : editing} onClose={() => { setEditingTarget(null); void load() }} onSaved={saved => { setEditingTarget(null); setMessage(saved.status === 'published' ? 'Post published. Your changes are live.' : 'Draft saved. Only you can see it.'); void load() }} />
  const visible = posts.filter(post => (filter === 'all' || post.status === filter) && `${post.title} ${post.category}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => sort === 'title' ? a.title.localeCompare(b.title) : sort === 'reads' ? Number(stats?.find(s => s.post_id === b.id)?.total_views || 0) - Number(stats?.find(s => s.post_id === a.id)?.total_views || 0) : b.updated_at.localeCompare(a.updated_at))
  return <section><div className="studio-heading"><div><p className="eyebrow">ASTROBITPLAYS</p><h1>Dashboard</h1></div><button className="button" onClick={() => { setMessage(''); setEditingTarget('new') }}><Plus size={18} /> New post</button></div>
    ...
    <div className="stats-grid"><div><span>Published posts</span><strong>{loading || !postsLoaded ? '—' : posts.filter(p => p.status === 'published').length}</strong></div><div><span>Drafts</span><strong>{loading || !postsLoaded ? '—' : posts.filter(p => p.status === 'draft').length}</strong></div><div><span>Article reads</span><strong>{loading || !stats ? '—' : stats.reduce((sum, item) => sum + Number(item.total_views), 0)}</strong></div><div><span>Reads · last 30 days</span><strong>{loading || !stats ? '—' : stats.reduce((sum, item) => sum + Number(item.recent_views), 0)}</strong></div></div>
    ...
    <div className="posts-toolbar"><h2>Your posts</h2><div className="post-filters"><label><span className="sr-only">Search your posts</span><input type="search" placeholder="Search your posts…" value={search} onChange={e => setSearch(e.target.value)} /></label><label><span className="sr-only">Sort posts</span><select value={sort} onChange={e => setSort(e.target.value)}><option value="updated">Recently updated</option><option value="title">Title A–Z</option><option value="reads">Most read</option></select></label><label>Show<select value={filter} onChange={e => setFilter(e.target.value)}><option value="all">All posts</option><option value="draft">Drafts</option><option value="published">Published</option></select></label></div></div>
    {loading ? <p role="status">Loading posts…</p> : visible.length ? <div className="table-wrap"><table><thead><tr><th>Title</th><th>Category</th><th>Status</th><th>Reads</th><th>Updated</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map(post => <tr key={post.id}><td>{post.title}</td><td>{post.category}</td><td><span className={`post-status ${post.status}`}>{post.status}</span></td><td>{stats ? Number(stats.find(item => item.post_id === post.id)?.total_views || 0) : '—'}</td><td>{formatDate(post.updated_at)}</td><td><div className="table-actions"><button className="text-link" onClick={() => { setMessage(''); setEditingTarget(post) }}><Pencil size={15} /> Edit<span className="sr-only"> {post.title}</span></button>{post.status === 'published' && <a className="text-link" href={`/?article=${encodeURIComponent(post.slug)}`} target="_blank" rel="noreferrer"><ExternalLink size={15} /> View</a>}</div></td></tr>)}</tbody></table></div> : <div className="dashboard-empty"><h3>{search ? 'No matching posts' : filter === 'all' ? 'Your first post starts here' : `No ${filter} posts`}</h3><p>{search ? 'Try another title or category.' : filter === 'all' ? 'Write a story, save it as a draft, and publish when it’s ready.' : 'Posts with this status will appear here.'}</p></div>}
  </section>
}
function Editor({ userId, post, onClose, onSaved }: { userId: string; post?: Post; onClose: () => void; onSaved: (post: Post) => void }) {
  ...
  const [recovered] = useState(() => {
    try {
      ...
      const legacy = sessionStorage.getItem(post ? `astrobit_draft_${post.id}` : 'astrobit_draft_new')
      if (!legacy) return null
      return decodeBackup(JSON.stringify({ version: 1, form: JSON.parse(legacy), baseUpdatedAt: post?.updated_at || null, savedAt: new Date().toISOString() }))
    } catch { return null }
  })
  const [form, setForm] = useState<PostInput>(recovered?.form || postInput(post))
  const baseUpdatedAt = recovered ? recovered.baseUpdatedAt : post?.updated_at || null
  const [dirty, setDirty] = useState(!!recovered), [busy, setBusy] = useState(false), [uploading, setUploading] = useState(false), [inlineUploading, setInlineUploading] = useState(false), [preview, setPreview] = useState(false), [error, setError] = useState('')
  const [backupStatus, setBackupStatus] = useState(recovered ? 'Recovered unsaved changes from this tab.' : 'No unsaved changes.')
  useEffect(() => {
    if (!dirty) return
    try {
      sessionStorage.setItem(storageKey, JSON.stringify({ version: 1, form, baseUpdatedAt, savedAt: new Date().toISOString() }))
      // This state reports the outcome of writing to browser storage.
      // oxlint-disable-next-line react/set-state-in-effect
      setBackupStatus('Unsaved changes backed up in this tab. Save to sync them to your account.')
    } catch { setBackupStatus('Browser backup unavailable. Save your draft before leaving.') }
  }, [form, dirty, storageKey, baseUpdatedAt])
  useEffect(() => {
    function shortcut(event: KeyboardEvent) {
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') {
        event.preventDefault()
        if (!busy && !uploading && !inlineUploading && !preview) formRef.current?.requestSubmit()
      }
    }
  ...
  function removeBackup() {
    try {
      ...
      sessionStorage.removeItem(post ? `astrobit_draft_${post.id}` : 'astrobit_draft_new')
    } catch { /* A failed storage operation must not fail a database save. */ }
  }
  function downloadDraft() {
    const url = URL.createObjectURL(new Blob([`# ${form.title}\n\n${form.excerpt}\n\n${form.body}`], { type: 'text/markdown;charset=utf-8' }))
    const link = document.createElement('a')
    link.href = url; link.download = `${form.slug || 'draft'}.md`; link.click()
  ...
  async function uploadInlineImage(file: File | undefined) {
    ...
    try {
      ...
      insertFormat(markdownImage(file.name.replace(/\.[^/.]+$/, ''), url), '', '')
  ...
  async function save(event: FormEvent) {
    ...
    setBusy(true)
    try {
      const result = post ? await database().from('posts').update(payload).eq('id', post.id).eq('updated_at', baseUpdatedAt!).select('*').single() : await database().from('posts').insert(payload).select('*').single()
      ...
      removeBackup(); setDirty(false); onSaved(result.data as Post)
  ...
  function close() {
    if (!dirty || window.confirm('Discard unsaved changes?')) {
      ...
      removeBackup(); onClose()
    }
  }
  return <section><div className="editor-heading"><button className="back-link" onClick={close} disabled={busy || uploading || inlineUploading}><ArrowLeft size={16} /> All posts</button><button className="button secondary" disabled={busy || uploading || inlineUploading} onClick={() => setPreview(!preview)}><Eye size={17} /> {preview ? 'Back to editor' : 'Preview'}</button></div><h1>{post ? 'Edit post' : 'New post'}</h1><div className="draft-status"><p role="status">{backupStatus}</p><button type="button" className="text-link" onClick={downloadDraft}>Download draft</button></div>
    ...
    : <form ref={formRef} className="editor-form" onSubmit={save}><fieldset disabled={busy} className="writing-area">
      ...
      <div className="article-field"><label htmlFor="article-body">Article</label>
        <div className="editor-toolbar" role="toolbar" aria-label="Markdown formatting">
          ...
          <label className="toolbar-btn upload-btn" title="Upload and insert image inline" style={{ cursor: 'pointer' }}><ImageIcon size={14} /> {inlineUploading ? 'Uploading…' : 'Image'}<input type="file" accept="image/png,image/jpeg,image/webp" disabled={inlineUploading || busy} className="sr-only" onChange={e => { void uploadInlineImage(e.target.files?.[0]); e.target.value = '' }} /></label>
          <button type="button" className="toolbar-btn" onClick={() => insertFormat('\n\n', '\n\n', 'Paste a YouTube URL here')} title="Embed YouTube Video"><Video size={14} /> YouTube</button>
          <button type="button" className="toolbar-btn" onClick={() => insertFormat('\n\n', '\n\n', 'Paste an X post URL here')} title="Embed X / Tweet"><Share2 size={14} /> X / Tweet</button>
        </div>
        <textarea id="article-body" ref={bodyRef} className="body-editor" required maxLength={200000} value={form.body} onChange={e => change('body', e.target.value)} />
        <small>Markdown supported: ## headings, **bold**, *italic*, links and lists. Use Preview to check formatting.</small>
      </div>
    </fieldset><fieldset disabled={busy} className="publishing-options">
      ...
      <button className="button" disabled={busy || uploading || inlineUploading}>{busy ? 'Saving…' : form.status === 'published' ? post?.status === 'published' ? 'Save changes' : 'Publish post' : post?.status === 'published' ? 'Unpublish and save draft' : 'Save draft'}</button>
