...
const url =
  ...
  "https://yodtnppcsmyeymbnmvsu.supabase.co";
const key =
...
export async function trackView(postId: string) {
  try {
    ...
    if (!visitor) {
      ...
      sessionStorage.setItem("abp-visit", visitor);
    }
    const key = `abp-read-${postId}-${new Date().toISOString().slice(0, 10)}`;
    if (sessionStorage.getItem(key)) return;
    const { error } = await database().rpc("record_post_view", {
      post_id: postId,
      visitor_id: visitor,
    });
    if (!error) sessionStorage.setItem(key, "1");
  } catch {
    /* Reading remains available when analytics or browser storage is blocked. */
  }
}
