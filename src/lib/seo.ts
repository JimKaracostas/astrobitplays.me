...
export const siteDescription = "Gaming news and reviews from AstroBitPlays.";
export const articleUrl = (slug: string, category = "News") =>
  `${siteUrl}${categoryPath(category)}${encodeURIComponent(slug)}/`;
const organization = {
  "@type": "Organization",
  name: "AstroBitPlays",
  url: siteUrl,
            ...
            : page === "signin"
              ...
              : isSearch
                ? query
                  ? `Search: ${query}`
                  : "Search articles"
                : section || "Gaming news & reviews");
  const canonical = post
    ? articleUrl(post.slug, post.category)
    : slug
      ? `${siteUrl}/?article=${encodeURIComponent(slug)}`
      : validSection
        ? `${siteUrl}${categoryPath(section)}`
        : `${siteUrl}/`;
  const description = post
    ? plainDescription(post.excerpt || post.body)
    : validSection
      ? `The latest gaming ${section.toLowerCase()} from AstroBitPlays.`
      : siteDescription;
  const image = new URL(
    post?.cover_url ? safeImage(post.cover_url) : placeholder,
    siteUrl,
  ).href;
  const schema =
    noindex || (slug && !post)
      ...
      : post
        ...
        : {
            "@context": "https://schema.org",
            "@type": validSection ? "CollectionPage" : "WebSite",
            name: title,
            url: canonical,
            description,
            publisher: organization,
...
export function applyMetadata(metadata: ReturnType<typeof pageMetadata>) {
  ...
  if (metadata.schema) {
    ...
    script.id = "publication-schema";
    script.type = "application/ld+json";
    script.textContent = JSON.stringify(metadata.schema);
